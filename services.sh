...
# Bases take the reserved band from its first slot, stepping by 1000: lane ports
# are base + slot*100, so 9000/9100/9200… stays ten lanes clear of 10000. Both are
# clear of AirPlay's :5000/:7000. Labels are kept SHORT — `obs status` renders them
# in a fixed ~12-col field and a long one breaks the table alignment.
SERVICE_TABLE=(
  # "key|repo-key|label|base-port|log-stem|health-path"
  "be|be|backend|9000|teacher-backend|/health"
  "fe|fe|frontend|10000|teacher-frontend|/"
  #   A third service would take 11000, and so on.
)
